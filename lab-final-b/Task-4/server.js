...
const applyDiscount = (req, res, next) => {
    ...
    res.locals.originalTotal = total;
    res.locals.discount = discount;
    res.locals.finalTotal = discountedTotal;
...
app.post('/admin/orders/:id/status', async (req, res) => {
    try {
        ...
        let isValid = false;
        if (currentStatus === 'Placed' && newStatus === 'Processing') isValid = true;
        if (currentStatus === 'Processing' && newStatus === 'Delivered') isValid = true;
...
app.get('/order/preview', applyDiscount, (req, res) => {
    const cart = req.session.cart || [];
    ...
    res.render('order-preview', {
        title: 'Order Preview',
        cart,
        total: res.locals.originalTotal,
        discount: res.locals.discount,
        finalTotal: res.locals.finalTotal,
        couponCode: res.locals.couponCode
    });
});

app.post('/order/confirm', applyDiscount, async (req, res) => {
    try {
        const cart = req.session.cart || [];
        if (cart.length === 0) return res.redirect('/cart');

        const { customerEmail } = req.body;
        if (!customerEmail) return res.status(400).send('Email is required');

        const newOrder = new Order({
            items: cart,
            totalAmount: res.locals.finalTotal,
...
app.get('/my-orders', (req, res) => {
    res.render('my-orders', { title: 'My Order History', orders: null, email: '' });
});

app.post('/my-orders', async (req, res) => {
    try {
        const { email } = req.body;
        if (!email) return res.render('my-orders', { title: 'My Order History', orders: null, email: '', error: 'Email is required' });

        const orders = await Order.find({ customerEmail: email }).sort({ createdAt: -1 });
        res.render('my-orders', { title: 'My Order History', orders, email });
    } catch (err) {
        console.error(err);
